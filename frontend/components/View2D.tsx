...
export const View2D: React.FC<View2DProps> = ({ dimensions, thickness, svgUrl }) => {
  const { width, height, depth } = dimensions;

  if (svgUrl) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center bg-slate-100 dark:bg-[#0d1117] relative overflow-hidden">
        <div className="absolute top-4 left-4 z-10 px-3 py-1.5 bg-white/90 dark:bg-slate-800/90 backdrop-blur rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm">
          <span className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wide">Arquivo de Corte (SVG)</span>
        </div>
        <iframe
          src={svgUrl}
    ...
    <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 dark:from-[#1e2736] dark:to-[#151c2a] p-12 overflow-auto">
      ...
      <div className="relative w-full h-full flex items-center justify-center">
        <div className="absolute top-6 left-6 flex items-center gap-2 px-3 py-1.5 bg-white/80 dark:bg-slate-800/80 backdrop-blur rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm pointer-events-none">
          ...
          <span className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wide">Preview Esquemático (Gere a caixa para ver o corte real)</span>
        </div>
